...
*/
		...
		case IKS_AST_CHAMADA_DE_FUNCAO:
						...
						LeftSon = stack_pointer->disc->scc[0]; // filho correspondente à função

						FunctionSearch = list_lookup(function_list,LeftSon->symbol->text);

						if(FunctionSearch != NULL){
					
							AllParams = FunctionParamList(AllParams,FunctionSearch->tam); puts("*****");
							list_print(AllParams);puts("*****");
						break;
						}
						else{
...
int inference(int val1, int val2)
{
	...
	if(val1==IKS_INT && val2==IKS_INT)
		...
		return IKS_FLOAT;
	else if((val1==IKS_STRING && val2!=IKS_STRING) || (val2==IKS_STRING && val1!=IKS_STRING)){
		printf("Impossivel fazer coerção para o tipo String (linha %d)\n", getLineNumber()-1);
		exit(IKS_ERROR_STRING_TO_X);	
	}
	else if((val1==IKS_CHAR && val2!=IKS_CHAR) || (val2==IKS_CHAR && val1!=IKS_CHAR)){
