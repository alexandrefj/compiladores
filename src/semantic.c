...
List* FunctionListInsert(List* function_list,DICT_NODE* node, int type){
	...
	if(list_lookup(function_list,node->text)==NULL){
		function_list=list_insert(function_list,type,node->text);
	} 
	else{
...
List* GlobalVarListInsert(List* list,DICT_NODE* node,int control,int type){

	switch(control){

		case GLOBAL_VAR_DEC_IDENTIFIER_CONTROL:		if(list_lookup(list,node->text)==NULL)
									list=list_insert(list,type,node->text); 
								else{
...
List* LocalVarListInsert(List* local_var,DICT_NODE* node, int type){
	...
	if(list_lookup(local_var,node->text)==NULL){
		local_var=list_insert(local_var,type,node->text);
	} 
	else{
	...
	if(stack_pointer!=NULL){ 
		...
		case IKS_AST_VETOR_INDEXADO:	
						...
						if(VectorialSearch !=NULL){
							flag = 2;
							stack_pointer->disc->scc[0]->node_type = VectorialSearch->type;
							stack_pointer->disc->scc[0]->size = var_size(stack_pointer->disc->scc[0]->node_type);
							break;
...
int inference(int val1, int val2)
{
	...
	if(val1==IKS_FLOAT && val2==IKS_FLOAT)
		return IKS_FLOAT;

	if(val1==IKS_BOOL && val2==IKS_BOOL)
		return IKS_BOOL;

	if((val1==IKS_INT && val2==IKS_FLOAT) || (val2==IKS_INT && val1==IKS_FLOAT))
		return IKS_FLOAT;

	if((val1==IKS_INT && val2==IKS_BOOL) || (val2==IKS_INT && val1==IKS_BOOL))
		return IKS_INT;

	if((val1==IKS_BOOL && val2==IKS_FLOAT) || (val2==IKS_BOOL && val1==IKS_FLOAT))
		return IKS_FLOAT;
